...
public class UserMealsUtil {
    public static void main(String[] args) {
        ...
        System.out.println();
        List<UserMealWithExcess> mealsToByStreams = filteredByStreams(meals, LocalTime.of(0, 0), LocalTime.of(23, 59), 2000);
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        for (UserMeal meal : meals) {
            ...
            int caloriesWithMeal = calories.getOrDefault(mealDate, 0) + meal.getCalories();
            calories.put(mealDate, caloriesWithMeal);
        }
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        return meals.stream().collect(Collectors.collectingAndThen(Collectors.groupingBy(meal -> meal.getDateTime().toLocalDate(), Collectors.summingInt(UserMeal::getCalories)), map -> meals.stream().flatMap(meal -> {
            if (TimeUtil.isBetweenHalfOpen(meal.getDateTime().toLocalTime(), startTime, endTime)) {
                return Stream.of(new UserMealWithExcess(meal.getDateTime(), meal.getDescription(), meal.getCalories(), map.get(meal.getDateTime().toLocalDate()) > caloriesPerDay));
            }
            return null;
